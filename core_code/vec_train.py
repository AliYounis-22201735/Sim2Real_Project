#!/usr/bin/env python3
"""
PPO training script for vector-based models
...
Usage Examples:
    ...
    python vec_train.py --continue-training --model-path <model_file>
    
    # Continue training from saved model with specific timesteps
    python vec_train.py --continue-training --model-path <model_file> --timesteps <timesteps>
...
from image_vectorizer import VectorizedDonkeyEnv
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Train PPO model for vector-based DonkeyCar control',
...
class TrainingCycleConfig:
    ...
    def __init__(self):
        ...
        self.GAE_LAMBDA = 0.95 # Factor for trade-off between bias and variance
        ...
        self.VF_NET_ARCH = [256, 64] # Value function network architecture
        self.ORTHO_INIT = True # Use orthogonal initialization
        self.LOG_STD_INIT = -2.0 # Initial log standard deviation


        # Directory Paths
        self.MODEL_PATH = "./models/ppo_vector" # Directory to save vector-based trained models
        self.LOG_PATH = "./logs/ppo_vector" # Directory for saving training logs
        self.CHECKPOINT_PATH = "./checkpoints/ppo_vector" # Directory for saving model checkpoints
...
def create_env(env_id=cfg.ENV_ID, n_envs=cfg.NUMBER_OF_ENVS, conf=sim_conf):
    ...
    def _make_env():
        ...
        try:
            ...
            env = VectorizedDonkeyEnv(env_id, conf=conf)

            # 2. Apply time limit wrapper
            env = TimeLimit(env, max_episode_steps=cfg.MAX_EPISODE_STEPS)

            # 3. Apply horizontal flipping, in case configured
    ...
    # 4. Create vectorized environment(s)
    env_manager = [_make_env for _ in range(n_envs)]
    env = DummyVecEnv(env_manager)

    # 5. Apply frame stacking
    env = VecFrameStack(env, n_stack=cfg.STACK_SIZE)

    # 6. Apply monitoring wrapper
    env = VecMonitor(env)

    # 7. Return the final vectorized environment(s)
    return env
...
def main():
    """Main training function for vector-based models"""
    print("🔄🔄🔄🔄🔄 CALLFLOW: Entering vector_train.py - PPO Vector Training Pipeline 🔄🔄🔄🔄🔄")
    ...
    if args.continue_training:
        ...
        if args.model_path:
            ...
            print("Usage: python vec_train.py --continue-training --model-path path/to/model.zip")
            sys.exit(1)
    ...
    run_name = f"ppo_vector_{run_time}"
    ...
    print(f"\n🚀  Starting DonkeyCar Vector Training...")
    ...
    if cfg.CONTINUE_TRAINING and cfg.MODEL_FILE:
        ...
        try:
            ...
            print(f"📊 Current timesteps in loaded model: {ppo_vector_model.num_timesteps:,}")
            print(f"🎯 Will train for additional {cfg.TOTAL_TIMESTEPS:,} timesteps")
            print(f"📈 Total timesteps after training: {ppo_vector_model.num_timesteps + cfg.TOTAL_TIMESTEPS:,}")
            print("=" * 50 + "\n")
        except Exception as e:
            print(f"❌ Failed to load model from {cfg.MODEL_FILE}")
            print(f"Error: {e}")
            print("⚠️  Creating new model instead...")
            ppo_vector_model = PPO(
                policy="MlpPolicy",
                env=train_env,
                verbose=1,
        ...
        print("=" * 50 + "\n")
        ppo_vector_model = PPO(
            policy="MlpPolicy", # Vector-based observations
            env=train_env,
            verbose=1,
            tensorboard_log=cfg.TENSORBOARD_PATH,
            device=device,
    ...
    try:
        ...
        # Save final model
        ppo_vector_model.save(f"{run_path}/final_model")

        # Save training cycle summary
        training_duration = time.time() - training_start_time
        cfg.save_config(run_name, "Training completed successfully!", device, run_path, ppo_vector_model.num_timesteps, training_duration)

        print("\n" + "=" * 50)
        print("🎉 PPO Vector Training Completed Successfully!")
        print(f"⏰  Total Training Time: {training_duration // 3600:.0f}h {(training_duration % 3600) // 60:.0f}m {training_duration % 60:.0f}s")
        print(f"🐾  Timesteps: {ppo_vector_model.num_timesteps:,}")
        ...
        try:
            ppo_vector_model.save(f"{run_path}/interrupted_model")
            training_duration = time.time() - training_start_time
            cfg.save_config(run_name, "Training interrupted by user!", device, run_path, ppo_vector_model.num_timesteps, training_duration)
        ...
        print(f"\n🚨 Training failed: {e}")
        try:
            ppo_vector_model.save(f"{run_path}/failed_model")
            training_duration = time.time() - training_start_time
            cfg.save_config(run_name, f"Training failed due to: {str(e)}", device, run_path, ppo_vector_model.num_timesteps, training_duration)
